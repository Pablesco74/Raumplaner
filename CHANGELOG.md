...
## 2026-09-25
- Nachbesserung: Eingabefeld der editierbaren Maßlinien (Möbel-Kantenabstand, Tür/Fenster-Eckabstand) war trotz `getScreenCTM()`-Fix (siehe Eintrag oben) weiterhin sichtbar versetzt - besonders deutlich, wenn beim Fokussieren die Bildschirmtastatur aufklappt: Das per CSS-Pixel-Koordinaten separat positionierte `<input>` wurde beim dadurch ausgelösten Layout-Reflow (Canvas verkleinert sich) nicht neu positioniert, da seine Position nur einmalig beim Öffnen berechnet wurde. Eingabefeld wird jetzt direkt an Ort und Stelle bearbeitet: Es ersetzt die Maßzahl per `<foreignObject>` innerhalb des SVG selbst (statt separat über CSS-Pixel-Koordinaten positioniert zu werden) - dadurch exakt an derselben Stelle und in derselben Skalierung wie der Rest des Grundrisses, bleibt bei jedem Reflow automatisch korrekt ausgerichtet. Nebenbei behobene Spezifitäts-Kollision: Die allgemeine Sidebar-Regel `input[type="number"] { width: 90px }` überschrieb wegen höherer CSS-Spezifität die feste 30px-Breite des Eingabefelds.
- Dateien: style.css, js/interaction.js, index.html
